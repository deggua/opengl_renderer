...
struct TextureRT : Handle<GLuint> {
    ...
    void BindMS(GLenum texture_slot) const;
    void UnbindMS(GLenum texture_slot) const;

    void Setup(GLenum format, GLsizei width, GLsizei height);
    void SetupMS(GLenum format, GLsizei samples, GLsizei width, GLsizei height);
};
...
struct Image2D : Handle<GLuint> {
    ...
    void BindImage(GLuint image_slot, GLenum access = GL_READ_WRITE) const;
...
struct FBO : Handle<GLuint> {
    ...
    void AttachMS(TextureRT tex_rt, GLenum attachment) const;
    void Attach(Image2D img_rt, GLenum attachment) const;
